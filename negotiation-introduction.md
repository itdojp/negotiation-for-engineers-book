...
### 技術選定の場面
新しいプロジェクトで React か Vue.js かを選ぶ。マイクロサービスにするかモノリスで始めるか。クラウドベンダーは AWS か GCP か。これらの決定は純粋に技術的な問題ではない。チームの学習コスト、既存資産との整合性、将来の拡張性など、多角的な視点からの説得が必要である。
...
### 予算獲得の場面
パフォーマンス改善、セキュリティ強化、開発環境の改善。これらは直接的な売上に結びつかないため、投資対効果の説明が求められる。たとえば「レスポンスタイムが0.5秒短縮される」という技術的成果から、対象traffic、baseline conversion、1件当たり粗利、効果の観測期間を明示した**検証可能な仮説**へ変換する。改善量だけから売上効果を断定せず、悲観・中位・楽観の幅と検証計画を示す必要がある。
...
Edmondson（1999）の心理的安全性は、対人的なリスクを取っても安全だという、チーム成員に共有された信念である（[一次論文](https://doi.org/10.2307/2666999)）。本書では、質問、異論、誤り、不確実性を表明しても侮辱や報復で応じない会話条件として用いる。快適さ、無条件の同意、対立の不在、低い品質基準を意味せず、一回の交渉から個人やチームを診断しない。定義、対象、限界は[基礎語彙のSource Notes](docs/foundations/index.md#source-notes-negotiation-foundations)を参照する。

交渉の場でこの会話条件を保つための実践的なchecklist：
...
### 継続的な学習
交渉力は、一朝一夕に身につくものではない。本書の内容を実践し、結果を振り返り、改善を重ねることで、着実に向上する。GitHub リポジトリ（itdojp/negotiation-for-engineers-book）では、追加のリソースやコミュニティでの議論を提供している。
