...
あるエンジニアは、複数の外部APIを統合する新システムの提案で苦戦していた。技術仕様書では「API連携により業務効率が向上する」と説明したが、経営層の反応は鈍かった。この状況の根本原因を分析すると、以下の問題が浮かび上がる：

**現状分析：なぜ提案が通らないのか**

1. **抽象的な効果説明**: 「業務効率が向上する」という表現は、具体的な数値や体験を伴わない
2. **技術的複雑性**: API連携の技術的メリットが、非技術者には理解困難
3. **投資対効果の不明確性**: 開発コストに対して、どの程度の効果が期待できるかが曖昧
...
## 実践演習：プロトタイプとデータによる説得（WIP: Issue #2対応）

本章で学んだプロトタイプとデータドリブンな提案手法を実践するための演習問題を提供する。読者が自身の業務課題に適用し、具体的な説得資料を作成するプロセスを体験する。

- **プロトタイプ作成演習**: 自身の提案を可視化するための簡易プロトタイプを設計・実装する。
- **データ分析演習**: 提案のビジネス価値を定量化するためのデータ収集・分析・可視化を行う。

## ケーススタディ：交渉成功・失敗事例集（WIP: Issue #2対応）

実際のエンジニアリング現場で発生した交渉の成功事例と失敗事例を詳細に分析する。各事例において、どのような交渉戦略が取られ、それがどのような結果をもたらしたのかを解説し、読者が自身の経験と照らし合わせて学べるようにする。

- **成功事例**: 困難な技術投資の承認、大規模リファクタリングの推進、チーム間の対立解消など。
- **失敗事例**: 技術的優位性のみを主張し、ビジネスサイドの理解を得られなかったケース、感情的な対立によりプロジェクトが停滞したケースなど。
